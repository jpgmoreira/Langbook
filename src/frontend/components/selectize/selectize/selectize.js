...
!(function (root, factory) {
    ...
    (Sifter.prototype.tokenize = function (query) {
        ...
        (Sifter.prototype.getScoreFunction = function (search, options) {
            function scoreValue(score, token) {
                var pos;
                return !score || -1 === (pos = (score = String(score || '')).search(token.regex)) ? 0 : ((score = token.string.length / score.length), 0 === pos && (score += 0.5), score);
            }
            var field_count,
        ...
        (Sifter.prototype.prepareSearch = function (query, options) {
            ...
            return option_fields && !is_array(option_fields) && (options.fields = [option_fields]), option_sort && !is_array(option_sort) && (options.sort = [option_sort]), option_sort_empty && !is_array(option_sort_empty) && (options.sort_empty = [option_sort_empty]), { options: options, query: String(query || '').toLowerCase(), tokens: this.tokenize(query), total: 0, items: [] };
        }),
                    ...
                    (Interface.plugins = {}),
                        (Interface.prototype.initializePlugins = function (plugins) {
                            var i,
                                ...
                                key,
                                queue = [];
                            if (((this.plugins = { names: [], settings: {}, requested: {}, loaded: {} }), utils.isArray(plugins))) for (i = 0, n = plugins.length; i < n; i++) 'string' == typeof plugins[i] ? queue.push(plugins[i]) : ((this.plugins.settings[plugins[i].name] = plugins[i].options), queue.push(plugins[i].name));
    ...
    (function (root, factory) {
        ...
        function measureString(str, $parent) {
            return str
                ? (Selectize.$testInput || ((Selectize.$testInput = $('<span />').css({ position: 'absolute', width: 'auto', padding: 0, whiteSpace: 'pre' })), $('<div />').css({ position: 'absolute', width: 0, height: 0, overflow: 'hidden' }).append(Selectize.$testInput).appendTo('body')),
        ...
        var IS_MAC = /Mac/.test(navigator.userAgent),
            ...
            Selectize = function ($input, settings) {
                ...
                if (
                    ((dir = (dir = dir ? dir.getPropertyValue('direction') : input.currentStyle && input.currentStyle.direction) || $input.parents('[dir]:first').attr('dir') || ''),
                    $.extend(this, {
                        order: 0,
                        settings: settings,
                        $input: $input,
        ...
        return (
            ...
            $.extend(Selectize.prototype, {
                setup: function () {
                    var delimiterEscaped,
                        ...
                        $control_input = $('<input type="text" spellcheck="false" autocomplete="new-password" autofill="no" />')
                            .appendTo($control)
                    ...
                    (inputId = $input.attr('id')) && ($control_input.attr('id', inputId + '-selectized'), $("label[for='" + inputId + "']").attr('for', inputId + '-selectized')),
                        ...
                        self.plugins.names.length && ((delimiterEscaped = 'plugin-' + self.plugins.names.join(' plugin-')), $wrapper.addClass(delimiterEscaped), selector.addClass(delimiterEscaped)),
                        ...
                        !self.settings.splitOn && self.settings.delimiter && ((delimiterEscaped = self.settings.delimiter.replace(/[-\/\\^$*+?.()|[\]{}]/g, '\\$&')), (self.settings.splitOn = new RegExp('\\s*' + delimiterEscaped + '+\\s*'))),
                ...
                setupCallbacks: function () {
                    var key,
                        fn,
                        callbacks = { initialize: 'onInitialize', change: 'onChange', item_add: 'onItemAdd', item_remove: 'onItemRemove', clear: 'onClear', option_add: 'onOptionAdd', option_remove: 'onOptionRemove', option_clear: 'onOptionClear', optgroup_add: 'onOptionGroupAdd', optgroup_remove: 'onOptionGroupRemove', optgroup_clear: 'onOptionGroupClear', dropdown_open: 'onDropdownOpen', dropdown_close: 'onDropdownClose', type: 'onType', load: 'onLoad', focus: 'onFocus', blur: 'onBlur', dropdown_item_activate: 'onDropdownItemActivate', dropdown_item_deactivate: 'onDropdownItemDeactivate' };
                ...
                refreshOptions: function (triggerDropdown) {
                    ...
                    for (
                        ...
                        (optgroup = groups_order[i]), self.optgroups.hasOwnProperty(optgroup) && groups[optgroup].childNodes.length ? ((html_children = document.createDocumentFragment()).appendChild(self.render('optgroup_header', self.optgroups[optgroup])), html_children.appendChild(groups[optgroup]), html.appendChild(self.render('optgroup', $.extend({}, self.optgroups[optgroup], { html: ((d = html_children), (tmp = void 0), (tmp = document.createElement('div')).appendChild(d.cloneNode(!0)), tmp.innerHTML), dom: html_children })))) : html.appendChild(groups[optgroup]);
            ...
            (Selectize.count = 0),
            (Selectize.defaults = { options: [], optgroups: [], plugins: [], delimiter: ',', splitOn: null, persist: !0, diacritics: !0, create: !1, showAddOptionOnCreate: !0, createOnBlur: !1, createFilter: null, highlight: !0, openOnFocus: !0, maxOptions: 1e3, maxItems: null, hideSelected: null, addPrecedence: !1, selectOnTab: !0, preload: !1, allowEmptyOption: !1, showEmptyOptionInDropdown: !1, emptyOptionLabel: '--', closeAfterSelect: !1, scrollDuration: 60, deselectBehavior: 'previous', loadThrottle: 300, loadingClass: 'loading', dataAttr: 'data-data', optgroupField: 'optgroup', valueField: 'value', labelField: 'text', disabledField: 'disabled', optgroupLabelField: 'label', optgroupValueField: 'value', lockOptgroupOrder: !1, sortField: '$order', searchField: ['text'], searchConjunction: 'and', mode: null, wrapperClass: 'selectize-control', inputClass: 'selectize-input', dropdownClass: 'selectize-dropdown', dropdownContentClass: 'selectize-dropdown-content', dropdownParent: null, copyClassesToDropdown: !0, render: {} }),
            ($.fn.selectize = function (settings_user) {
                ...
                return this.each(function () {
                    ...
                    this.selectize ||
                        ...
                        settings.allowEmptyOption && settings.showEmptyOptionInDropdown && !$input.children('option[value=""]').length && ((input_html = $input.html()), $input.html('<option value="">' + settings.emptyOptionLabel + '</option>' + input_html)),
                        ('select' === tag_name
                            ? init_select
                            : function (value, settings_element) {
                              ...
                              })($input, (settings_element = { placeholder: settings_element, options: [], optgroups: [], items: [] })),
            ...
            Selectize.define('auto_position', function () {
                ...
                this.positionDropdown = function () {
                    ...
                    position === POSITION.top ? (Object.assign(styles, { bottom: offset.top, top: 'unset', margin: '0 0 5px 0' }), this.$dropdown.addClass('selectize-position-top')) : (Object.assign(styles, { top: offset.top, bottom: 'unset', margin: '5px 0 0 0' }), this.$dropdown.removeClass('selectize-position-top')), this.$dropdown.css(styles);
                };
            }),
